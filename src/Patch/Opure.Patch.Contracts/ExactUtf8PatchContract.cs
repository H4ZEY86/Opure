...
public sealed class ExactUtf8PatchProposal
{
        ...
        ReadOnlySpan<byte> contentUtf8)
    {
        ...
        ProposalSha256 = ComputeProposalSha256();
    }
    ...
    private string ComputeProposalSha256()
    {
        ...
        Append(hash, PatchId);
        Append(hash, ProjectId);
        Append(hash, RootReferenceId);
        Append(hash, BaseWorkspaceGeneration.ToString(System.Globalization.CultureInfo.InvariantCulture));
        Append(hash, BaseWorkspaceGenerationSha256);
        Append(hash, TargetPathReferenceId);
        Append(hash, OperationKind.ToString());
        Append(hash, ExpectedSourceSha256 ?? string.Empty);
        Append(hash, ExpectedSourceSizeBytes?.ToString(System.Globalization.CultureInfo.InvariantCulture) ?? string.Empty);
        Append(hash, LineEndingIntent.ToString());
        Append(hash, CreatorKind.ToString());
        Append(hash, IntentSummary);
        Append(hash, CreatedAtUtc.ToString("O", System.Globalization.CultureInfo.InvariantCulture));
        Append(hash, ResultingContentSha256);
