...
/// <summary>
/// A per-call, disposable gRPC client for the Patch Review service over named-pipe transport.
/// Callers must dispose after each call. No state is retained between calls.
/// </summary>
public sealed class NamedPipePatchReviewClient : IAsyncDisposable
{
    ...
    private readonly PatchReview.PatchReviewClient client;

    public NamedPipePatchReviewClient(
        ...
        int? clientProcessId = null)
    {
        ...
        {
            HttpHandler = handler,
            MaxSendMessageSize = PatchReviewContractPolicy.MaximumRequestBytes,
            MaxReceiveMessageSize = PatchReviewContractPolicy.MaximumResponseBytes
        });
        client = new PatchReview.PatchReviewClient(channel);
    }

    public Task<GetActivePatchesResponse> GetActivePatchesAsync(
        GetActivePatchesRequest request, CancellationToken cancellationToken) =>
        InvokeAsync(
            "GetActivePatches",
            request,
            (headers, deadline) => client.GetActivePatchesAsync(request, headers, deadline, cancellationToken),
            cancellationToken);

    public Task<GetPatchPreviewResponse> GetPatchPreviewAsync(
        GetPatchPreviewRequest request, CancellationToken cancellationToken) =>
        InvokeAsync(
            "GetPatchPreview",
            request,
            (headers, deadline) => client.GetPatchPreviewAsync(request, headers, deadline, cancellationToken),
            cancellationToken);

    public Task<ApprovePatchResponse> ApprovePatchAsync(
        ApprovePatchRequest request, CancellationToken cancellationToken) =>
        InvokeAsync(
            "ApprovePatch",
            request,
            (headers, deadline) => client.ApprovePatchAsync(request, headers, deadline, cancellationToken),
            cancellationToken);

    public Task<CancelPatchResponse> CancelPatchAsync(
        CancelPatchRequest request, CancellationToken cancellationToken) =>
        InvokeAsync(
            "CancelPatch",
            request,
            (headers, deadline) => client.CancelPatchAsync(request, headers, deadline, cancellationToken),
            cancellationToken);
        ...
        where TResponse : class
    {
        ...
        try
        {
            ...
            if (!RuntimeHealthSessionAuthentication.VerifyServerProof(
                    ...
                    responseHeaders))
            {
                throw new PatchReviewTransportException(
...
public sealed class PatchReviewTransportException(
    string errorCode,
    string safeMessage,
