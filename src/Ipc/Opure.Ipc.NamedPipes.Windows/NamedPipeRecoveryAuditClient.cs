...
public sealed class NamedPipeRecoveryAuditClient : IAsyncDisposable
{
    ...
    private readonly RecoveryOrchestrator.RecoveryOrchestratorClient client;

    public NamedPipeRecoveryAuditClient(
        ...
        int? clientProcessId = null)
    {
        ...
        client = new RecoveryOrchestrator.RecoveryOrchestratorClient(channel);
    }
    ...
    public Task<GetUnresolvedAuditsResponse> GetUnresolvedAuditsAsync(
        GetUnresolvedAuditsRequest request, CancellationToken cancellationToken) =>
        InvokeAsync(
            "GetUnresolvedAudits",
            request,
            (headers, deadline) => client.GetUnresolvedAuditsAsync(request, headers, deadline, cancellationToken),
            cancellationToken);

    public Task<RestoreSnapshotResponse> RestoreSnapshotAsync(
        RestoreSnapshotRequest request, CancellationToken cancellationToken) =>
        InvokeAsync(
            "RestoreSnapshot",
            request,
            (headers, deadline) => client.RestoreSnapshotAsync(request, headers, deadline, cancellationToken),
            cancellationToken);

    public Task<DiscardSnapshotResponse> DiscardSnapshotAsync(
        DiscardSnapshotRequest request, CancellationToken cancellationToken) =>
        InvokeAsync(
            "DiscardSnapshot",
            request,
            (headers, deadline) => client.DiscardSnapshotAsync(request, headers, deadline, cancellationToken),
            cancellationToken);
