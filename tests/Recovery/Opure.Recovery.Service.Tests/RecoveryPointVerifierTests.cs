...
public sealed class RecoveryPointVerifierTests : IDisposable
{
    ...
    public async Task VerifyRecoveryPointAsync_ReturnsFalse_WhenHashesMismatch()
    {
        ...
        string testChannel = $"Test-{epochId:N}";
        ...
        // Updated per FND-060 schema migration:
        var manifest = new RecoveryPointManifest(epochId, epoch, "local", testChannel, owners,
            ProductVersion: null,
            SupportedSchemas: Array.Empty<uint>().AsReadOnly(),
            CheckpointHashes: Array.Empty<string>().AsReadOnly(),
            VerificationLevel.None,
            CreationTimestamp: DateTimeOffset.UtcNow,
            CreatorId: null,
            VerificationReceipts: Array.Empty<EvidenceReceipt>().AsReadOnly());
        var adapters = Array.Empty<IBackupAdapter>();

        // Act
        bool result = await RecoveryPointVerifier.VerifyRecoveryPointAsync(manifest, backupDir, testChannel, adapters, CancellationToken.None);
    ...
    public async Task VerifyRecoveryPointAsync_ReturnsTrue_AndDeletesDisposableRoot()
    {
        ...
        string stagingBase = Path.Combine(localAppData, "Opure", testChannel, "Staging", "Recovery");
